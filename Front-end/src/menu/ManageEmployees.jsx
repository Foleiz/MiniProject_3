            ...
            <div className="modal-content">
                ...
                <div className="modal-body">
                    ...
                    <div className="form-group"><label>ตำแหน่ง*</label>
                        <select name="POSITIONID" onChange={handleChange}><option value="">กรุณาเลือก</option>{positions.map((p) => (<option key={p.POSITIONID} value={p.POSITIONID}>{p.POSITIONNAME}</option>))}</select>
                        {errors.POSITIONID && <span className="error">{errors.POSITIONID}</span>}
                    </div>
                    <div className="form-group"><label>แผนก*</label>
                        <select name="DEPARTMENTID" onChange={handleChange}><option value="">กรุณาเลือก</option>{departments.map((d) => (<option key={d.DEPARTMENTID} value={d.DEPARTMENTID}>{d.DEPTNAME}</option>))}</select>
        ...
        <div className="modal-overlay">
            <div className="modal-content">
                ...
                <div className="modal-body">
                    ...
                    <div className="form-group"><label>ตำแหน่ง*</label>
                        <select name="POSITIONID" value={form.POSITIONID || ""} onChange={handleChange}><option value="">กรุณาเลือก</option>{positions.map((p) => (<option key={p.POSITIONID} value={p.POSITIONID}>{p.POSITIONNAME}</option>))}</select>
                        {errors.POSITIONID && <span className="error">{errors.POSITIONID}</span>}
                    </div>
                    <div className="form-group"><label>แผนก*</label>
                        <select name="DEPARTMENTID" value={form.DEPARTMENTID || ""} onChange={handleChange}><option value="">กรุณาเลือก</option>{departments.map((d) => (<option key={d.DEPARTMENTID} value={d.DEPARTMENTID}>{d.DEPTNAME}</option>))}</select>
                        {errors.DEPARTMENTID && <span className="error">{errors.DEPARTMENTID}</span>}
                    </div>
...
export default function ManageEmployees() {
    ...
    const loadData = async () => {
        const e = await fetch(`${API_BASE_URL}/api/employees`).then(r => r.json());
        const p = await fetch(`${API_BASE_URL}/api/positions`).then(r => r.json());
        const d = await fetch(`${API_BASE_URL}/api/departments`).then(r => r.json());
        setEmployees(e);
        setPositions(p);
        setDepartments(d);
    };
    ...
    const handleAdd = async (data) => {
        ...
        await fetch(`${API_BASE_URL}/api/employees/new`, {
            method: "POST",
    ...
    const handleEdit = async (emp) => {
        ...
        await fetch(`${API_BASE_URL}/api/employees/db/${emp.EMPLOYEEID}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
        });
    ...
    const handleDelete = async (id) => {
        ...
        await fetch(`${API_BASE_URL}/api/employees/db/${id}`, { method: "DELETE" });
        await loadData();
    };
