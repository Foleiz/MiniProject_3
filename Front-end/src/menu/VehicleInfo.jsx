...
export default function VehicleInfo() {
  ...
  const fetchBusTypes = async () => {
    try {
      const res = await fetch("http://localhost:3000/api/bus_type");
      const data = await res.json();
  ...
  const handleAdd = async () => {
    await fetch("http://localhost:3000/api/buses", {
      method: "POST",
  ...
  const handleUpdate = async () => {
    await fetch(`http://localhost:3000/api/buses/${form.BusID}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(form)
    });
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`http://localhost:3000/api/buses/${id}`, { method: "DELETE" });
    fetchBuses();
  };
